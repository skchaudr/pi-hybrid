...
impl Bridge {
    ...
    pub async fn with_timeout(command: &str, timeout_duration: Duration) -> anyhow::Result<Self> {
        if command.trim().is_empty() {
            error!("Bridge command cannot be empty");
            anyhow::bail!("bridge command cannot be empty");
        }

        debug!(%command, "Starting bridge process");
    ...
    pub async fn call(&mut self, method: &str, params: Value) -> anyhow::Result<Value> {
        ...
        trace!(id, method, "Bridge JSON-RPC call");
        ...
        if bytes == 0 {
            error!(id, method, "Bridge closed stdout before responding");
        ...
        if let Some(error) = response.error {
            error!(%error, method, "Bridge returned error");
...
impl Drop for Bridge {
    fn drop(&mut self) {
        trace!("Dropping bridge, killing child process");
